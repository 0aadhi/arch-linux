...
drawbar(Monitor *m)
{
    ...
    strftime(dtext, sizeof(dtext), datetimestr, localtime(&now));

    w = TEXTW(dtext);
    drw_setscheme(drw, scheme[SchemeNorm]);
    drw_text(drw, x, 0, w, bh, lrpad / 2, dtext, 0);
 ...
 /* int boxs = drw->fonts->h / 9;
   ...
   // drw_text(drw, 0, 0, w, bh, lrpad / 2, m->ltsymbol, 0);
    ...
    int tag_width = 0;
    /* calculate centered position for layout + tags */
    int tagw = 0;
    for (i = 0; i < LENGTH(tags); i++)
        tagw += TEXTW(tags[i]);

    int layoutw = TEXTW(m->ltsymbol);
    int centerw = tagw + layoutw;
    int cx = (m->ww - centerw) / 2;

    /* draw layout symbol (center) */
   // drw_setscheme(drw, scheme[SchemeNorm]);
   // drw_text(drw, cx, 0, layoutw, bh, lrpad / 2, m->ltsymbol, 0);
   ...
   // x = cx + layoutw; 
    ...
    /* Draw tags centered */
    x = cx; // Start x at the centered position
    for (i = 0; i < LENGTH(tags); i++) {
        w = TEXTW(tags[i]);
    ...
    /* draw layout symbol AFTER tags */
    drw_setscheme(drw, scheme[SchemeNorm]);
    drw_text(drw, x, 0, layoutw, bh, lrpad / 2, m->ltsymbol, 0);
